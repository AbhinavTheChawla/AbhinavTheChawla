...
const GroomingJournal = () => {
  ...
  const renderTable = (category, title, emoji, hasNumbers = false, isWishlist = false) => {
    ...
    return (
      <div className="mb-8">
        <div className="text-sm font-semibold uppercase text-slate-600 mb-3 tracking-wide flex items-center gap-2">
          <span>{emoji}</span>
          <span>{title}</span>
        </div>
        <div className="bg-white rounded-lg border border-slate-200 overflow-hidden shadow-sm">
          <table className="w-full">
            <thead className="bg-slate-50">
              <tr>
                {hasNumbers && <th className="p-3 text-left text-xs font-semibold text-slate-500 w-12">#</th>}
                <th className="p-3 text-left text-xs font-semibold text-slate-500">Product</th>
                <th className="p-3 text-left text-xs font-semibold text-slate-500">Details</th>
                {isWishlist && <th className="p-3 text-left text-xs font-semibold text-slate-500 w-32">Priority</th>}
                <th className="p-3 text-center text-xs font-semibold text-slate-500 w-16"></th>
              </tr>
            </thead>
            <tbody>
              {rows.map((row, i) => (
                <tr key={i} className="border-t border-slate-100 hover:bg-slate-50 transition-colors">
                  {hasNumbers && (
                    <td className="p-3 text-center text-slate-400 font-medium text-sm">{i + 1}</td>
                  )}
                  <td className="p-3">
                    <input
                      type="text"
                      ...
                      className="w-full border border-transparent hover:border-slate-300 focus:border-indigo-400 focus:ring-2 focus:ring-indigo-100 rounded px-2 py-1.5 text-sm transition-all"
                  ...
                  {isWishlist && (
                    <td className="p-3">
                      <input
                        ...
                        className="w-full border border-transparent hover:border-slate-300 focus:border-amber-400 focus:ring-2 focus:ring-amber-100 bg-amber-50 rounded px-2 py-1.5 text-sm transition-all"
        ...
        <button
          ...
          className="mt-2 text-sm text-indigo-600 hover:text-indigo-700 font-semibold flex items-center gap-1 transition-colors"
        >
          <Plus size={14} />
  ...
  return (
    <div className="bg-white/80 backdrop-blur-sm rounded-2xl shadow-xl border border-slate-200 p-8">
      <div className="mb-6">
        <h2 className="text-2xl font-bold text-gray-800">Grooming Routine Tracker</h2>
        <p className="text-slate-500 mt-1 text-sm">Click any cell to edit • Changes save automatically</p>
      </div>
