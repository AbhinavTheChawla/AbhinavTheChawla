...
const Media = () => {
  ...
  // Local state
  const [activeSection, setActiveSection] = useState('tracker'); // 'tracker', 'toConsume', 'recap', 'recovery'
...
`;
  ...
  // Recovery functions - get all consumed items grouped by week
  const getAllConsumedByWeek = () => {
    const weekMap = new Map();

    safeMediaData.consumed.forEach(item => {
      const key = `${item.year}-W${item.weekNumber}`;
      if (!weekMap.has(key)) {
        weekMap.set(key, {
          year: item.year,
  ...
  // Generate recap for a specific week
  const generateRecapForWeek = (weekNumber, year) => {
    // Get all items from specified week
    const weekItems = safeMediaData.consumed.filter(
      item => item.weekNumber === weekNumber && item.year === year
    );

    if (weekItems.length === 0) {
      alert(`No media found for Week ${weekNumber}, ${year}`);
      return;
    }
    ...
    // Use the earliest and latest consumedDate from the items
    const dates = weekItems.map(item => item.consumedDate).sort((a, b) => a - b);
    ...
    if (existingRecapIndex >= 0) {
      ...
      newRecaps = [...safeMediaData.weeklyRecaps];
      newRecaps[existingRecapIndex] = recap;
      alert(`Updated existing recap for Week ${weekNumber}, ${year}`);
    } else {
      // Add new recap
      newRecaps = [recap, ...safeMediaData.weeklyRecaps];
      alert(`Created recap for Week ${weekNumber}, ${year} with ${weekItems.length} items`);
    }
    ...
    // Switch to recap section to show the result
        ...
        <div className="space-y-4">
          <div className="bg-white rounded-xl p-6 shadow-md">
            <h2 className="text-xl font-bold mb-4 text-slate-800 flex items-center gap-2">
              <Calendar size={20} />
              Data Recovery - All Past Weeks
            </h2>
            <p className="text-sm text-slate-600 mb-4">
              This section shows all your consumed media grouped by week, including data from previous weeks that may not be visible in the current week's tracker.
              You can generate weekly recaps for any past week to preserve your data.
            </p>

            {safeMediaData.consumed.length === 0 ? (
              <p className="text-slate-500 text-center py-8">No consumed media found in storage</p>
            ) : (
              <div className="space-y-4">
                <div className="bg-green-50 border border-green-200 rounded-lg p-4">
                  <p className="text-sm text-green-800 font-medium mb-2">
                    Found {safeMediaData.consumed.length} total items in storage
                  </p>
                  <p className="text-xs text-green-700">
                    Current week is Week {getCurrentWeek()}, {new Date().getFullYear()}
                  </p>
                </div>

                {getAllConsumedByWeek().map(weekData => {
                  const isCurrentWeek = weekData.weekNumber === getCurrentWeek() && weekData.year === new Date().getFullYear();
                  const hasRecap = safeMediaData.weeklyRecaps.some(
                    r => r.weekNumber === weekData.weekNumber && r.year === weekData.year
                  );

                  return (
                    <div
                      key={`${weekData.year}-W${weekData.weekNumber}`}
                      className={`border rounded-lg p-4 ${
                        isCurrentWeek
                          ? 'border-blue-300 bg-blue-50'
                          : 'border-slate-200 bg-white hover:shadow-md transition-all'
                      }`}
                    >
                      <div className="flex items-center justify-between mb-3">
                        <div>
                          ...
                          <p className="text-sm text-slate-600">
                            {weekData.items.length} item{weekData.items.length !== 1 ? 's' : ''}
                          </p>
                        </div>
                        <button
                          onClick={() => generateRecapForWeek(weekData.weekNumber, weekData.year)}
                          className={`px-4 py-2 ${
                            hasRecap
                        ...
                        >
                          <Calendar size={16} />
                          {hasRecap ? 'Update Recap' : 'Create Recap'}
                        </button>
                      </div>
                            ...
                            <div className="flex items-start justify-between gap-3">
                              <div className="flex-1">
                                <a
                                  ...
                                  target="_blank"
                                  rel="noopener noreferrer"
                                  className="text-green-600 hover:text-green-800 font-medium flex items-center gap-2 mb-1 text-sm"
                                >
                                  {item.title}
                                  <ExternalLink size={12} />
                                </a>
